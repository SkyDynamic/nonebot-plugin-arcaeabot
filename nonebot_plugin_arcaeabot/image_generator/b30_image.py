...
def draw_score_detail(
    score_info: Dict, song_info: Dict, rank: int, mask: Image.Image
) -> Image.Image:
    # Frame
    diff: int = score_info["difficulty"]
    diffculty: float = song_info["rating"] / 10    
    cover_name = "3.jpg" if diff == 3 else "base.jpg"
    song_background = open_img(
        StaticPath.song_dir / score_info["song_id"] / cover_name
    ).resize((270, 270))
    ...
    write_score = DataText(45, 80, 40, f'{score_info["score"]:,}', StaticPath.exo_medium)
    ...
    write_rating = DataText(
        320, 148, 25, f'{score_info["rating"]:.3f}', StaticPath.kazesawa_regular
    )
...
def draw_user_b30(data: Dict):
    ...
    # User Info
    best: float = data["best30_avg"]
    recent: float = data["recent10_avg"]
    ...
    score_info_list = (
        data["best30_list"] + data["best30_overflow"]
    )
    song_info_list = (
        data["best30_songinfo"] + data["best30_overflow_songinfo"]
    )
    ...
    for num, value in enumerate(score_info_list):
        ...
        B30_bg.alpha_composite(
            draw_score_detail(
                score_info=score_info_list[num], song_info=song_info_list[num], rank=num, mask=mask
            ),
